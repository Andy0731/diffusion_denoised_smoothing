...
class ApproximateAccuracy(Accuracy):
    ...
    def at_radius(self, df: pd.DataFrame, radius: float):
        # df["predict"] = 'tensor([' + df["predict"].astype(str) + '])'
        # return((df["predict"] == df["label"]) & (df["radius"] >= radius)).mean()
...
if __name__ == "__main__":
    ctf_files = [
        ...
        # preporduce carlini code
        "amlt/dds/diffusion_denoise_smoothing_reproduce_n2/reproduce/certify_0.25_mod"

    ]

    plot_certified_accuracy(
        "amlt/dds/analysis/plots/rep_carlini_025", "rep_carlini_025", 1.0, [
